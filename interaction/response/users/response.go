...
// GetFavoritesListByFavoriteVideoResponse params：
// al:用户创建的所有收藏夹
// ids：这个视频存在于哪些收藏夹内（不局限于当前用户创建的）
func GetFavoritesListByFavoriteVideoResponse(al *favorites.FavoriteList, ids []uint) (data interface{}, err error) {
	...
	for _, v := range *al { // al是FavoritesList的列表
		coverInfo := new(common.Img)
		err = json.Unmarshal(v.Cover, coverInfo) //json=>struct
		cover, _ := conversion.FormattingJsonSrc(v.Cover)
		//判断是否已选  对用户创建的每个收藏夹都判断一次视频是否被收藏在这个收藏夹里
		selected := false
		for _, vv := range ids {
			if vv == v.ID {
				selected = true
		...
		list = append(list, GetFavoritesListByFavoriteVideoInfo{
			...
			Max:      v.Max,
			Selected: selected,
			Present:  len(v.CollectList), //返回的是一个收藏夹的长度，即收藏夹内已收藏视频数
			UsesInfo: struct {
				Username string `json:"username"`
			}{
				Username: v.UserInfo.Username,
