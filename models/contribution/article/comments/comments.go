...
func (cl *CommentList) GetCommentListByIDs(ids []uint, info common.PageInfo) error {
	return global.Db.Debug().Where("article_id", ids).Preload("UserInfo").Preload("ArticleInfo").Limit(info.Size).Offset((info.Page - 1) * info.Size).Order("created_at desc").Find(&cl).Error
}
