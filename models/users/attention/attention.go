...
// GetNewAddAttentionByTime 获取从指定时间过后的新增关注数
func (at *Attention) GetNewAddAttentionByTime(time string, uid uint) (int64, error) {
	var count int64
	err := global.Db.Model(&Attention{}).Where("uid = ?", uid).Where("created_at > ?", time).Count(&count).Error
	if err != nil {
