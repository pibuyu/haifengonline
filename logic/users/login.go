...
func writeMessageToKafka(data string) error {
	_, err := global.NormalProducer.WriteMessages(kafka.Message{Value: []byte(fmt.Sprintf("用户名为%s的用户登陆了", data))})
	if err != nil {
		global.Logger.Errorf("写入消息出错：%v", err)
		return err
	}
	//global.Logger.Infof(fmt.Sprintf("用户名为%s的用户登陆了", data))
	return nil
}
...
func Login(data *receive.LoginReceiveStruct) (results interface{}, err error) {
	...
	global.Logger.Infof("用户 %s 在%s登陆", users.Username, time.Now().Format(time.DateTime))
	...
	if err != nil {
		return nil, fmt.Errorf("创建notice记录错误")
	}
	return userInfo, nil
}
