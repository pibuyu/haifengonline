...
// Start 启动服务,监听视频观看者的加入和退出
func (e *Engine) Start() {
	for {
		select {
		//注册事件
		case registerMsg := <-e.Register:
			global.Logger.Infof("打印registerMsg信息:%v", registerMsg)
			//添加成员
		...
		case cancellationMsg := <-e.Cancellation:
			global.Logger.Infof("打印cancellationMsg信息:%v", cancellationMsg)
			//广播在线观看人数
...
func (lre VideoRoomEvent) Writer() {
	for {
		...
		case msg := <-lre.Channel.MsgList:
			global.Logger.Infof("打印接收到的消息:%v", msg)
